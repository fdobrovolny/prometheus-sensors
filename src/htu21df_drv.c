...
static void htu21df_prometheus_metrics(struct mg_connection *nc,
                                       void *user_data) {
  struct mgos_htu21df_stats stats;

  mgos_prometheus_metrics_printf(nc, GAUGE, "temperature",
                                 "Temperature in Celcius",
                                 "{sensor=\"0\",type=\"HTU21DF\"} %f",
                                 mgos_htu21df_getTemperature(s_htu21df));
  mgos_prometheus_metrics_printf(nc, GAUGE, "humidity",
                                 "Relative humidity percentage",
                                 "{sensor=\"0\",type=\"HTU21DF\"} %f",
                                 mgos_htu21df_getHumidity(s_htu21df));
...
void htu21df_drv_init() {
  ...
  LOG(LL_WARN, ("HTU21DF and SI7021 are both on I2C address 0x40 -- do not "
                "enable both!"));
#endif
  s_htu21df = mgos_htu21df_create(
      mgos_i2c_get_global(), mgos_sys_config_get_sensors_htu21df_i2caddr());
  if (s_htu21df) {
    mgos_set_timer(mgos_sys_config_get_sensors_htu21df_period() * 1000, true,
                   htu21df_timer_cb, NULL);
